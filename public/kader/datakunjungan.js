...
const DatKunjunganController = (function(URL, UI) {
    ...
    const printReport = (id_kegiatan) => {
        $.ajax({
            url: `${BASE_URL}master/kader/Kunjungan/print_daftar_kunjungan?id_kegiatan=` + id_kegiatan,
            method: 'get',
            dataType: 'json',
            success: function(data){
                ...
                if (data.file_path) {
                    ...
                    alert('File could not be generated.');
                }
            }
        })
    }
    ...
    $(".print-kunjungan-report").on('click', function() {
        console.log(id_kegiatan);
        printReport(id_kegiatan);
    });
