    ...
    const renderListAnak = (data) => {
        ...
        if(data.length > 0){
            data.forEach((item) => {
                ...
                html += `
                    <tr> 
                        <td> ${item.no_kk_anak} </td>
                        <td> ${item.no_kk} </td>
                        <td> ${nama_lengkap} </td>
                        <td> ${item.jenis_kelamin} </td>
                        <td> <button type="button" id="pilih__anak" data-no_kk_anak="${item.no_kk_anak}" data-nama="${nama_lengkap}" > Pilih Anak </button> </td>
                    <tr>
                `;
            })
        }
            ...
            data.forEach(item => {
                html += `
                    <tr>
                        ...
                        <td> <a href="#/detailkms/${item.kk_anak_number}" class="btn btn-info" > Lihat Kunjungan </a> </td>
                    </tr>
                `;
            })
        }
...
const KmsController = (function(UI, URL) {
    ...
    const eventListener = () => {
        ...
        /**
         *  event keypress no.kk_anak 
         */
        $(DOM.form.add).on('keypress', DOM.field.no_kk_anak, function(e) {
        ...
        /**
         * show modal search KK Anak
         */
        $(DOM.button.searchKKAnak).on('click', () => {
            loadListAnak()
            ModalAction(DOM.modal.modalKKAnak, 'show')
        })

        /**
         * search on modal (list anak) 
         */
        $(DOM.modal.modalKKAnak).on('keyup', DOM.field.searchAnak, function() {
            if($(this).val() !== '') {
         ...
         * btn pilih anak
         */
        $(DOM.modal.modalKKAnak).on('click', DOM.button.pilihAnak, function() {
            var no_kk_anak = $(this).data('no_kk_anak')
            ModalAction(DOM.modal.modalKKAnak, 'hide')
            $(DOM.field.no_kk_anak).val(no_kk_anak);
        })
         ...
         */
         ...
         $(DOM.htmlshow.showKMS).on('click', DOM.button.editKms, function() {
             var id = $(this).data('id'), no_kk_anak = $(this).data('no_kk_anak') , nama = $(this).data('nama_lengkap'), bb   = $(this).data('bb'), pb = $(this).data('pb')
             $(DOM.field.nama_edit).val(nama)
             $(DOM.field.pb_edit).val(pb)
             $(DOM.field.bb_edit).val(bb)
             $(DOM.field.no_kms_edit).val(id)
             $(DOM.field.kk_anak_edit).val(no_kk_anak)
