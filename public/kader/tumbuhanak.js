...
const TumbuhAnakController = (function(URL, UI) {
    ...
    const eventListener = function(){
        ...
        $(dom.html.showKunjungan).on('click', dom.btn.pilihAnak, function() {
            ...
            $(dom.field.no_kk_anak).val($(this).data('no_kk_anak'))
            $(dom.field.no_kk).val($(this).data('no_kk'))
            ModalAction(dom.modal.listKunjungan, 'hide')
            $(dom.btn.btnHitungGizi).css('display','block')

            $.ajax({
                url:  BASE_URL+`master/kader/Anak/get_detail_anak_chart?no_kk_anak=${$(this).data('no_kk_anak')}`,
                method: 'GET',
                dataType: 'json',
                success: function (e) {
                    console.log(e);
                    ...
                    $('#growth_chart_container').css('display', 'flex');
